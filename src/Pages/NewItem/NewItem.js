...
const NewItem = () => {
  ...
  const addNewItem = async () => {
    try {
      await sendRequest('http://localhost:5000/api/clothes/newitem', 'POST', JSON.stringify({
  ...
  return (
    <>
      ...
      <Content>
        ...
        <ItemForm changeSelectValueHandler={changeSelectValueHandler} changeRadioValue={changeRadioValue} selectValue={selectValue}>
          <input type="file"/>
          </ItemForm>
      </Content>
      <ButtonsContainer>
        <MainButton onClick={addNewItem} disabled={!(selectValue && radioValue)}>
          ADD ITEM
        </MainButton>
      </ButtonsContainer>
    </>
